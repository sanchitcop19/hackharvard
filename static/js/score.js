//based on https://bl.ocks.org/mbostock/1096355
	...
	(function(){
		...
		function build(dataset,singleArcView, credit_score){
          ...
          if (credit_score){
            field.append("text").attr('class','goal').text("CREDIT SCORE").attr("transform","translate(0,50)");
          }		
           else{
            field.append("text").attr('class','goal').text("CONFIDENCE ").attr("transform","translate(0,50)");
           }
...
//		build(ranDataset);
		build(ranDataset2,true, true);
    build(ranDataset2,true, false);
